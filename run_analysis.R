...
zip_location <- "UCI HAR Dataset"
ZipFile <- "data.zip"
ResultFile <- "tidy_data.txt"
dataPath <- "./data"
ZipFilePath <- file.path(dataPath,ZipFile)
ResultFilePath <- file.path(dataPath,ResultFile)
...
if(!file.exists(ZipFilePath)){
        ...
        log("writing zip file: `",ZipFilePath,"`")
        fileHandle <- file(ZipFilePath, open="wb")
        writeBin(binaryData, fileHandle)  
        close(fileHandle)  
        rm(binaryData)
}else{
        log("zip file already exists: `",ZipFilePath,"`")
}

# unzip if not already exists
extractedZipPath <- file.path(dataPath, ZipFile)
codebook("* extracting zip file: `",ZipFilePath,"` to `",extractedZipPath,"`")
if(!file.exists(extractedZipPath) || debug){
        log("unzip file: `",ZipFilePath, "` to `",dataPath,"`")
        unzip(ZipFilePath, exdir=dataPath, overwrite=TRUE)
}else{
        log("zip file already extracted to: `",extractedZipPath,"`")
}

files <- list.files(extractedZipPath, recursive=TRUE)


# loading test and train file ignoring inertial folder
cleandir <- files[!grepl("Inertial", files) & grepl("test|train", files)]

codebook("* merging all *_test.txt and *_train.txt files into one dataset: `mergedData`")
if(!exists("mergedData") || debug){
        log("load .txt files:")
        for(files in cleandir){
                # generate parameters based on files
                param_Name <- paste0("data_", tolower(sub("^.*/([^\\.]+).*$","\\1",
                                                         files, perl=TRUE)))
                txtFile <- file.path(extractedZipPath, files)
